...
from proba import diagram

FONT = ("Arial", 20, "bold")
STATIONS = ["Station_1", "Station_2", "Station_3", "Station_4"]
TODAY_DATE = dt.datetime.now().strftime("%Y-%m-%d %H:%M")
...
def save():
    ...
    if len(station) == 0 or len(temp) == 0 or len(res) == 0:
        ...
        try:
            ...
            resistance_entry.delete(0, END)
            surface_stress_entry.delete(0, END)
...
diagram_button = Button(text="Get Data", width=10, command=diagram)
diagram_button.grid(column=1, row=1)
